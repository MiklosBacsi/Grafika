//=============================================================================================
// OpenGL keretrendszer: GLFW és GLAD alapu implementácio
//=============================================================================================
#include "framework.h"
#define GLFW_INCLUDE_NONE
#include <GLFW/glfw3.h>

// Keretrendszer allapota
...
// Rajzold ujra az alkalmazási ablakot
void glApp::refreshScreen() {
	screenRefresh = true;
}

// Lekerdezeses klaviatura kezeles
bool pollKey(int key) {
	return (glfwGetKey(window, key) == GLFW_PRESS);
}

int main(void) {
	// Alkalmazoi ablak letrehozasa
	...
	// Applikacio inicializalasa
	pApp->onInitialization();
	float startTime = 0;

	// Uzenetkezelo hurok
	while (!glfwWindowShouldClose(window)) {
		glfwPollEvents(); // esemenyek lekerdezése es reakcio

		float endTime = (float)glfwGetTime();    // ido lekerdezese
		pApp->onTimeElapsed(startTime, endTime); // animacio
		startTime = endTime;
